...
up — Xcode DerivedData, caches, logs, Homebrew/CocoaPods/gem leftovers, and the
Trash — with a checklist UI, per-task size estimates, and a report of the space
reclaimed.
...
(`brew`, `mas`, `pod`, `gem`, `xcrun`) grey out automatically.
...
- File deletions run through Swift's `FileManager`, not `rm -rf`, so a single
  protected file is skipped instead of aborting the whole run.
- Only user-level paths are touched (`~/Library/...`, `~/.Trash`). System-level
  `/Library/Logs` and the like are left alone.
- The app is **not sandboxed** — it needs to invoke `brew`/`mas`/`pod`/`gem`/
  `xcrun` and delete cache files in your home directory.
